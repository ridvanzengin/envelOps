...
def render_knowledge_query_instruction(
    ...
    instruction = (
        "Ground your reply ONLY in the knowledge listed below. This "
        "applies just as much to requests to DO something (modify, "
        "cancel, or exchange an existing order, for example) as it "
        "does to plain questions -- \"can you cancel my order\" is "
        "really asking whether the knowledge below describes how "
        "cancellations work, the same way a price question asks "
        "whether the knowledge below states a price. Decide which of "
        "these three situations applies, in this order:\n"
        "1. The knowledge below IS about the same general product, "
        "topic, or type of request being asked about, but the message "
        "is missing a detail you'd need before you could pick the "
        'right answer -- e.g. asking about the price/availability/'
        'color of "it" or "the red one" without saying which product, '
        "when the knowledge below does cover that kind of product, "
        ...
        "answer or explicitly describes how to do what's being asked "
        "-- respond using only that information.\n"
        ...
        "knowledge below does cover), or it's specific enough but the "
        "knowledge below doesn't actually cover it -- including "
        "specific facts like prices, policies, or guarantees, AND "
        "including how to carry out a request like a cancellation or "
        "exchange -- you MUST say you don't have that information and "
        "a person will confirm, rather than guessing or inferring an "
        "answer that merely sounds plausible. Never invent a next "
        "step, workflow, contact email, phone number, or policy that "
        "isn't stated in the knowledge below, even if it would sound "
        "helpful -- a confident but made-up answer is worse than "
        "admitting you don't know.\n"
        "Any line below marked [Live lookup result] came from a real, "
        "just-now live inventory or order-status check, not general "
        "business knowledge -- treat it as case 2 above and answer with "
        "it directly, even when it says something is out of stock, not "
        "currently available, or not something this business carries at "
        "all. That is still a complete, definite answer to the question "
        "asked, not a gap -- never say NOT_FOUND or that a person will "
        "confirm for something a live lookup result already answered. "
        "Never repeat the literal \"[Live lookup result]\" label itself "
        "in your reply -- it's only there to tell you the fact is "
        "authoritative, not something to quote to the customer.\n"
